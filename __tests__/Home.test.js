...
describe('Home Component', () => {
  ...
  it('calls onAuthStateChanged when component mounts', async () => {
    try {
      ...
      await waitFor(() => expect(require('../firebase').auth.onAuthStateChanged).toHaveBeenCalled());
    } catch (error) {
  ...
  it('passes correct user birthdays data to MonthCalendar component', async () => {
    try {
      ...
      await waitFor(() => {
        ...
        expect(monthCalendar.props.userBirthdays).toEqual([{ id: '1', name: 'John', day: 1 }, { id: '2', name: 'Jane', day: 5 }]);
      });
    } catch (error) {
